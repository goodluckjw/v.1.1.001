...
with st.expander("ℹ️ 사용법 안내"):
    st.markdown(      
        ...
        "     - <배제할 법률>에 입력된 법률은 개정문 생성 목록으로 반환되지 않습니다. 빈칸으로 두면 찾을 문자열이 포함된 모든 법률에 대해 개정문을 작성합니다. \n" 
        "     - <배제할 법률> 박스에서는 문자열의 공백을 무시합니다. (예. \"특정범죄 가중처벌 등에 관한 법률\"을 \"특정범죄가중처벌등에관한법률\"로 입력가능) 공백배제 기능은 <배제할 법률> 입력에만 적용됩니다. \n" 
        "- 이 앱은 현행 법률의 본문만을 검색 대상으로 합니다. 헌법, 폐지법률, 시행령, 시행규칙, 행정규칙, 제목, 부칙 등은 검색하지 않습니다. \n"
        "- 이 앱은 업무망에서는 작동하지 않습니다. 인터넷망에서 사용해주세요. \n"
        "- 가운뎃점(U+318D, 법제처 사이트에서 사용)을 입력해야 하는 경우 샵(#)으로 대체할 수 있습니다. (예. \"법률상#사실상의 주장\"을 입력하면 \"법률상ㆍ사실상의 주장\"으로 인식) \n"
        "- 검색기능의 <검색어 입력> 및 개정문 생성의 <바꿀 문자열>에는 공백이 있는 문자열을 입력할 때에도 큰따옴표가 필요하지 않습니다. \n"
        "- 법률 인용 기호 「」는 중괄호 { }로 입력할 수 있습니다. 예: {출입국관리법}에 관한 특례 → 「출입국관리법」에 관한 특례 \n"  # 추가
